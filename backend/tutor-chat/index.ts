...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = buildSystemPrompt(educationLevel, gradeLevel, fieldOfStudy, subjects, userName, learnerProfile);
    ...
    if (imageData && allMessages.length > 0) {
    ...
    const { contents, systemInstruction } = convertMessages(allMessages);
    ...
    const response = await fetch(
      getGeminiUrl(GOOGLE_AI_API_KEY, "gemini-2.0-flash", true),
      {
        method: "POST",
    ...
    if (!response.ok) {
      if (response.status === 429) {
        return errorResponse("Toki is taking a quick break. Please try again in a moment! 🌟", 429);
      }
      if (response.status === 402) {
        return errorResponse("Toki needs more energy to help. Please check back later! 💫", 402);
      }
      const errorText = await response.text();
      console.error("Gemini API error:", response.status, errorText);
      return errorResponse("Toki had a hiccup. Let's try that again! 🔄", 500);
    }
...
// Helper functions for building system prompts
...
function buildAdaptiveInstructions(profile?: LearnerProfile): string {
  if (!profile) return "";
...
function buildTopicContext(profile?: LearnerProfile): string {
  if (!profile) return "";
