...
serve(async (req) => {
  ...
  try {
    ...
    const educationContext = getEducationContext(educationLevel, gradeLevel, fieldOfStudy, subjects);

    // Handle diagram generation
    if (type === "diagram") {
      return await handleDiagramGeneration(GOOGLE_AI_API_KEY, content, diagramType);
    }

    // Handle summary generation
    if (type === "summary") {
      return await handleSummaryGeneration(GOOGLE_AI_API_KEY, content, isBase64);
    }

    // Handle flashcards and quiz generation
            ...
            properties: {
              flashcards: {
                ...
                items: {
                  type: "object",
                  properties: {
                    front: { type: "string", description: "The question or term to learn" },
    ...
    const geminiRequest = buildGeminiRequest({ messages, tools, toolChoice });
    ...
    return jsonResponse(generatedContent);

  } catch (e) {
    console.error("Generate content error:", e);
    return errorResponse(e instanceof Error ? e.message : "Unknown error occurred", 500);
  }
});
...
function getEducationContext(
  ...
  return `a ${gradeDescriptions[gradeLevel || 5] || gradeDescriptions[5]} student${subjectContext}. Use simple, child-friendly language with encouragement`;
}

async function handleDiagramGeneration(
  apiKey: string,
  content?: string,
  diagramType: "flowchart" | "mindmap" = "flowchart"
): Promise<Response> {
...
Remember: Return ONLY the Mermaid code, no explanations or markdown.`;
  ...
  const geminiRequest = buildGeminiRequest({ messages });

  const response = await fetch(
    getGeminiUrl(apiKey, "gemini-2.0-flash", false),
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(geminiRequest),
  ...
  if (!response.ok) {
    ...
    const errorText = await response.text();
    console.error("Gemini API error:", response.status, errorText);
    return errorResponse("Failed to generate diagram", 500);
  }

  const data = await response.json();
  let mermaidCode = extractTextContent(data);

  // Clean up the response - remove markdown code blocks if present
  mermaidCode = mermaidCode.replace(/```mermaid\n?/g, "").replace(/```\n?/g, "").trim();

  if (!mermaidCode) {
    return errorResponse("No diagram generated", 500);
  }

  return jsonResponse({ mermaidCode });
}
...
async function handleSummaryGeneration(
  ...
  const systemPrompt = "You are an expert at summarizing educational content. Create clear, concise summaries that capture the key points and main ideas. Format your summary with clear sections and bullet points where appropriate.";
  ...
  if (isBase64 && content) {
    ...
    const userPrompt = `Please summarize the following content. Provide a comprehensive summary with key points, main ideas, and important details. Use clear headings and bullet points where appropriate.\n\nContent to summarize:\n${content}`;
    messages = [
      { role: "system", content: systemPrompt },
      { role: "user", content: userPrompt }
    ];
  ...
  const data = await response.json();
  const summary = extractTextContent(data);

  if (!summary) {
    return errorResponse("No summary generated", 500);
  }
